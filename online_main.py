...
from pretrain_from_llm import get_new_dataset, get_llm_data_paths
...
if __name__ == "__main__":
    os.chdir("/gemini/code")
    hyperparams = {
        "env": "FrozenLake-v1",  # "CartPole-v0", "MountainCar-v0", "FrozenLake-v1", Pendulum-v1, "CliffWalking-v0", "RepresentedPong-v0"
        "seed": 42069,
        "n_episodes": 200,  # 5000,
        ...
        "n_exp": 5,
        "n_pretrain_eps": 20,
        "n_online_eps": 80,  # 10-290 for mountainCar, 30-120 for CartPole, 30-120 for FrozenLake, 30-570 for Pendulum
    ...
    if "Represented" in hyperparams["env"]:
        ...
        gym.make(hyperparams["env"]).observation_space, gym.spaces.Discrete
    ):
    ...
    os.makedirs("/gemini/output/cache", exist_ok=True)

    with open(
        f'/gemini/output/cache/cache_{hyperparams["env"].split("-")[0]}_Neps_{hyperparams["n_pretrain_eps"]}{suffix}.pkl',
        "wb",
    ) as file:
